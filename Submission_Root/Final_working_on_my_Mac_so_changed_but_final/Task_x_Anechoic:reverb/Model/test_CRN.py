...
import soundfile as sf  # Fixed for Mac stability
import numpy as np
from tqdm import tqdm

# Robust Import for metrics
try:
    ...
    print("Trying fallback imports for metrics...")
    from torchmetrics.functional.audio import stoi, pesq, scale_invariant_signal_distortion_ratio
    ShortTimeObjectiveIntelligibility = None
    PerceptualEvaluationSpeechQuality = None
    ScaleInvariantSignalDistortionRatio = None

# ==========================================
# 1. CONFIGURATION
# ==========================================
MODEL_PATH = "CRN_Reverb_finetuned.pth"
TEST_DATASET_ROOT = "/Users/parthdhamija/Desktop/LSTM_audio_zoomer/dataset_male_female_v3"
...
def run_evaluation():
    ...
    if PerceptualEvaluationSpeechQuality is None:
        print("Metric classes not found. Using simple fallback if available, or install torchmetrics[audio].")
        return
    ...
    print(f"Found {len(sample_folders)} samples. Starting evaluation...")
    ...
    best_folder = os.path.join(TEST_DATASET_ROOT, best_sample_name)
    ...
    try:
        ...
        with torch.no_grad():
            ...
            # Save using Soundfile to avoid torchaudio/MPS issues
            out_est_path = os.path.join(OUTPUT_DIR, "BEST_OVERALL_output.wav")
            out_mix_path = os.path.join(OUTPUT_DIR, "BEST_OVERALL_mixture.wav")
            out_tgt_path = os.path.join(OUTPUT_DIR, "BEST_OVERALL_target.wav")

            sf.write(out_est_path, est_trim.squeeze().cpu().numpy(), SAMPLE_RATE)
            sf.write(out_mix_path, mixture.squeeze().cpu().numpy().T, SAMPLE_RATE)
            sf.write(out_tgt_path, tgt_trim.squeeze().cpu().numpy(), SAMPLE_RATE)

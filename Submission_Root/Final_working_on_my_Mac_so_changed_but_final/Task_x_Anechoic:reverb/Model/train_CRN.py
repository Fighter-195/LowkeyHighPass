...
import torchaudio
# ==========================================
# 1. CONFIGURATION (MINI TEST MODE)
# ==========================================
# 1. PATH: Point to your new folder on the Desktop
DATASET_ROOT = r"/Users/parthdhamija/Desktop/LSTM_audio_zoomer/mini_dataset_v3" 

# 2. BATCH SIZE: Must be smaller than your dataset size (50)
BATCH_SIZE = 4       

# 3. EPOCHS: Set to 2 just to prove it works quickly
N_EPOCHS = 2         

# 4. WORKERS: Set to 0 to prevent "Broken Pipe" errors on Mac
NUM_WORKERS = 0      

# 5. DEVICE: Add Mac "MPS" support (or fallback to CPU)
# This makes it run faster on Mac chips (M1/M2/M3)
DEVICE = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
if str(DEVICE) == "cpu" and torch.cuda.is_available():
    DEVICE = torch.device("cuda")

# Standard Settings (Keep these the same)
LEARNING_RATE = 1e-3
PATIENCE = 10
N_FFT = 512
HOP_LENGTH = 160
...
class RoomAcousticDataset(Dataset):
    ...
    def _load_audio(self, path):
        # Force it to use 'soundfile' instead of 'torchcodec'

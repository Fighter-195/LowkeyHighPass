...
# ==========================================
DATASET_ROOT ="/Users/parthdhamija/Desktop/LSTM_audio_zoomer/dataset_male_female_v3"; 
PRETRAINED_PATH = "CRN_Model.pth"
NEW_MODEL_NAME = "CRN_Reverb_finetuned.pth" 
# ==========================================
# HYPERPARAMETERS (Mac & Mini-Dataset Tuned)
# ==========================================
# 1. BATCH SIZE: Reduced to 4 because you only have 50 samples. 
# (128 is too big and would crash immediately).
BATCH_SIZE = 4       

# 2. LEARNING RATE: Lowered to 1e-4. 
# We want to "gently" update the brain, not rewrite it entirely.
LEARNING_RATE = 1e-4      

# 3. EPOCHS: Set to 10. 
# Enough to learn the new voices, but fast enough to finish quickly.
N_EPOCHS = 3          

# 4. PATIENCE: Reduced to 3. 
# If it stops improving for 3 epochs, stop early.
PATIENCE = 3              

# 5. WORKERS: Set to 0. 
# REQUIRED for Mac to prevent "Broken Pipe" errors.
NUM_WORKERS = 0      

# 6. DEVICE: Auto-detects Mac (MPS), Nvidia (CUDA), or CPU.
DEVICE = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
if str(DEVICE) == "cpu" and torch.cuda.is_available():
    DEVICE = torch.device("cuda")

# Standard Audio Settings (Do not change these)
N_FFT = 512
HOP_LENGTH = 160
SILENCE_PROB = 0.3
